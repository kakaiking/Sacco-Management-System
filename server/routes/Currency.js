...
router.delete("/:id", validateToken, logDeleteOperation("Currency"), async (req, res) => {
  try {
    ...
    // Check if currency is being used by any products
    const { Products } = require("../models");
    const productsUsingCurrency = await Products.count({
      where: { currency: currency.currencyCode, isDeleted: 0 }
    });

    if (productsUsingCurrency > 0) {
      return res.status(400).json({ 
        error: `Cannot delete currency. It is being used by ${productsUsingCurrency} product(s).` 
      });
    }
